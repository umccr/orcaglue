...
The script uses `DROP TABLE IF EXISTS` followed by `CREATE TABLE` so an existing TSA table is recreated with the current metadata columns. Running it deletes any data currently stored in `tsa.csv__ica_usage_report`; **do not run it while the Glue job or downstream transformations are active**.
